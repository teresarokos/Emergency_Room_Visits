...
payments_charges <- emergency_visits_data %>% 
  ...
  mutate(rank = 1:5851) %>% 
  gather(-event_id, -condition, -`Total Expenditure`, -`Total Charge`, -`Total Facility Charge`, -`Total Doctor Charge`, -rank,
         -condition_2, -condition_3, -condition_4, key = payment_type, value = amount) %>%
  filter(case_when(`Total Expenditure` != 0 ~ amount != 0) | 
           case_when(`Total Expenditure` == 0 ~ payment_type %in% c("Total Facility Payment", "Total Doctor Payment"))) %>% 
  arrange(rank)
...
payments_charges %>% 
  ...
  geom_col() +
  scale_fill_manual(values = c("Out of Pocket" = "#F27D53", "Medicaid" = "#61B200", 
